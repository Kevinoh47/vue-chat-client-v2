  ...
  <v-app id="app" class="fill-height">
    <v-layout row class="fill-height" style="padding-bottom:60px" >
        <v-flex md8 offset-md2 style="overflow:auto;" class="pr-3 pl-3" v-if="HANDLE" ref="chatContainer">
          <div v-for="(chat, index) in CHATS" :key="index" class="mt-4 mb-4" style="max-width:80%">
        ...
        <v-flex md8 offset-md2 style="overflow:auto;" class="pr-3 pl-3" ref="chatContainer">
          <div v-for="(chat, index) in CHATS" :key="index" class="mt-4 mb-4" style="max-width:80%">
              <app-chat-item :chat="chat"></app-chat-item>
          </div>
        </v-flex>
...
export default {
  ...
  sockets : {
    ...
    incoming : function(val){
      console.log('incoming is the tag from yakker: ', val);
      //this.$store.dispatch("ADD_CHAT",val);
      //i suspect it just needs the latest string not the history, since it may be keeping its own history in the store...
      this.$store.dispatch("ADD_CHAT",val.payload);
    }
  }
}
